...
def pega_lista(dia, mes):
    db = sqlite3.connect('database.sqlite')
    cursor = db.cursor()
    evento = cursor.execute("SELECT evento FROM agenda where dia = '"+dia+"' and mes = '"+mes+"'")
    descricao = cursor.execute("SELECT descrição FROM agenda where dia = '"+dia+"' and mes = '"+mes+"'")
    return render_template("cria.html", evento=evento, descricao=descricao)
def login(dre,senha):
    db = sqlite3.connect('database.sqlite')
    cursor = db.cursor()
